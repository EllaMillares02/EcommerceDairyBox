    ...
    public function index(){
        ...
        if (Auth::check()) {
            ...
            if (Auth::user()->usertype == 1) {
                ...
                $expiring_soon_count = Product::whereBetween('expiry_date', [$today, $fiveDaysFromNow])->count();
                ...
                'total_user', 'expiring_soon_count', 'total_revenue', 'pending_orders_count', 'pickup_orders_count',
...
public function saveProfile(Request $request)
{
    ...
    return redirect('index')->with('success_message', 'Profile updated successfully!');
}
...
public function save_orders(Request $request)
{
    ...
    $pickupLocation = $request->input('meetup_location');
    ...
    $orderDetails .= '<br>Delivery Date: ' . $deliveryDate;
    ...
    if (Auth::id()) {
        ...
        $cancelOrdersCount = Order::where('user_id', $id)
           ...
           return view('home.orders', compact('orders', 'orderItems', 'orderItemspickup', 
                        'deliveryOrdersCount', 'pickupOrdersCount', 'orderComplete', 'completeOrdersCount', 'orderItemscancel', 'cancelOrdersCount', 'categories'));
...
public function view_spin(){
    $categories = Category::all();

    if (Auth::id()) {
    
    return view('home.view_spin', compact('categories')) ;
...
public function games(){
    ...
    return view('home.view_games', compact('categories', 'categories'));

}  
public function view_scratch(){
    $categories = Category::all();

    if (Auth::id()) {
    return view('home.view_scratch', compact('categories'));
...
public function view_quiz(){
    ...
    if (Auth::id()) {
    return view('home.quiz', compact('categories'));
