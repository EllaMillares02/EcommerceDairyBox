...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const deliveryDateContainer = document.getElementById('deliveryDate'); // Use getElementById

    // Initialize flatpickr for the delivery date
    const deliveryDatePicker = flatpickr(deliveryDateInput, {
        dateFormat: "Y-m-d",
        disable: [
            function(date) {
                // Disable all days except Thursdays (4) and Fridays (5)
                return !(date.getDay() === 4 || date.getDay() === 5);
            }
        ],
        minDate: "today", // Restrict past dates
        altInput: true, // Option to show a more human-readable format
        altFormat: "F j, Y" // e.g., October 20, 2024
    });
    ...
    function handleDeliveryTypeChange() {
        if (pickUpOption.checked) {
            ...
            pickupLocationInput.style.display = 'none'; // Hide meet-up location input
            pickupTimeInput.style.display = 'block'; // Show pick-up time input
            deliveryDateContainer.style.display = 'none'; // Hide delivery date input
            ...
            pickupLocationInput.style.display = 'block'; // Show meet-up location input
            pickupTimeInput.style.display = 'none'; // Hide pick-up time input
            deliveryDateContainer.style.display = 'block'; // Show delivery date input
...
document.getElementById('placeOrderForm').addEventListener('submit', function(event) {
    ...
    const pickupLocation = document.getElementById('location').value; // User's meet-up location
    const deliveryDateInput = document.getElementById('delivery-date');
    ...
    if (codOption.checked) {
        ...
        document.getElementById('hidden_meetup_location').value = "Pickup @ Albay Farmers' Bounty Village, Camalig, 4502 Albay"; // Store address for pick-up
